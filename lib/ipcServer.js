...
IPCServer = (function() {
  ...
  IPCServer.prototype.message = function(message) {
    ...
    if (!_.isFunction(func)) {
      return this.reply(message.id, message.func, message.type + 1, 'action not registered');
    }
    return func(ctx, message["arguments"], function(error, result) {
      return _this.reply(message.id, message.func, message.type + 1, error, result);
    });
  };
  ...
  IPCServer.prototype.reply = function(id, funcName, type, error, result) {
    var reply;
    if (type == null) {
